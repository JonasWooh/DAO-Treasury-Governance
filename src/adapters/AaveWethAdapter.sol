...
contract AaveWethAdapter is IAaveWethAdapter, ReentrancyGuard {
    ...
    function supply(uint256 amountWeth) external override onlyTreasury nonReentrant {
        if (amountWeth == 0) {
            revert ZeroAmount();
        }
    ...
    function withdraw(uint256 amountWeth) external override onlyTreasury nonReentrant {
        if (amountWeth == 0) {
            revert ZeroAmount();
        }
