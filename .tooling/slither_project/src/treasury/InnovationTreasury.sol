...
contract InnovationTreasury is IInnovationTreasury, Ownable {
    ...
    function releaseMilestone(bytes32 projectId, uint8 milestoneIndex, uint256 amountWeth) external override onlyOwner {
    ...
    function depositIdleFunds(uint256 amountWeth) external override onlyOwner {
        if (amountWeth == 0) {
            revert ZeroAmount();
        }
    ...
    function withdrawIdleFunds(uint256 amountWeth) external override onlyOwner {
        if (amountWeth == 0) {
            revert ZeroAmount();
        }
